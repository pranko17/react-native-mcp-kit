...
All tools that return heavy JSON (console, network, errors, storage, reactQuery, fiber_tree props/hooks, log_box, navigation, metro events, device) share the standard `path` / `depth` / `maxBytes` projection input. Slice is part of `path` syntax (`[-3:]`, `[10:20]`, etc.) — see `src/shared/projection/projectValue.ts` and `src/shared/projection/resolvePath.ts`. Heavy nested values collapse to `${kind}` markers (`{"${obj}":N}`, `{"${arr}":N}`, `{"${str}":{len,preview}}`, `{"${Date}":...}`, `{"${Err}":{name,msg}}`, `{"${cyc}":true}`, `{"${ref}":{mcpId,name}}`, etc.). Drill via `path: '[-1:][0].request.body.user.email'` from the same tool — no separate by-id fetcher.
