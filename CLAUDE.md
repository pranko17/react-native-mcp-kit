...
- **Capture** (`host/tools/capture.ts`): `host__screenshot` — WebP, auto-resized (default width 280), diff-cached via SHA-256 per device. Every fresh capture's metadata carries the `hash`; identical bytes return `{ unchanged: true, lastMeta }` where `lastMeta` is the meta of the previously-shipped image (width / height / originalWidth / originalHeight / scale / bytes / hash / region). Accepts `region: { x, y, width, height }` in original device pixels — crops BEFORE resize; pair with fiber bounds to snapshot one element for ~20-60 vision tokens. Response is `[image, metadataText]` where metadata JSON includes `{ width, height, originalWidth, originalHeight, scale, bytes, hash, region? }`. Routes per `resolved.device.kind`: iOS simulators → `xcrun simctl io screenshot`, Android → `adb exec-out screencap`, real iOS 17+ devices → `captureScreenshot` in `host/coredevice/` (CoreDevice tunnel + DTX `takeScreenshot`).
...
- **Devices** (`host/tools/devices.ts`): `host__list_devices({ connected? })` — annotates each device with `connected: true` / `clientId` when it matches a live client. Pass `connected: true` to drop devices without a live MCP client; per-platform error envelopes are preserved.
...
- **Symbolication** (`metro/tools/symbolicate.ts`): `metro__symbolicate({ stack? | frames?, metroUrl?, clientId?, maxFrames?, includeFrameworkFrames?, fullPaths? })` — POSTs to `/symbolicate`. Stack parser strips a leading `<ErrorName>: <message>` header line if present and accepts Hermes anonymous variants (bare `@file:L:C`, `anonymous@…`, and packager-rooted `?anon_N_/abs/path:L:C`). Drops `collapse: true` framework frames by default, caps to 10 frames, shortens absolute paths relative to cwd. Graceful no-op `{ skipped: true, error, frames }` when Metro is unreachable.
...
### Projection / slice pagination

All tools that return heavy JSON (console, network, errors, storage, reactQuery, fiber_tree props/hooks, log_box, navigation, metro events, device) share a uniform projection input via `makeProjectionSchema` — `path` / `depth` / `maxBytes` / `previewCap` / `objectCap` / `arrayCap`. Defaults: `previewCap=250` (strings collapse to `${str}` marker above this), `objectCap=30`, `arrayCap=50`, `maxBytes=50_000`, `depth` per-tool. Every knob has a machine-readable `default:` + `minimum:` in its inputSchema so agents see the exact bounds via `describe_tool`.

Path syntax (`.key`, `[N]`, `[a:b]`) works on arrays AND strings AND objects (Nth key / key slice). Ending a path with a slice on a string (`stack[0:500]`) returns the raw substring — `previewCap` is bypassed since the agent explicitly asked for a window; other paths that land on a string still respect `previewCap`. See `src/shared/projection/projectValue.ts` and `src/shared/projection/resolvePath.ts`. Heavy nested values collapse to `${kind}` markers (`{"${obj}":N}`, `{"${arr}":N}`, `{"${str}":{len,preview}}`, `{"${Date}":...}`, `{"${Err}":{name,msg}}`, `{"${cyc}":true}`, `{"${ref}":{mcpId,name}}`, etc.). Drill via `path: '[-1:][0].request.body.user.email'` from the same tool — no separate by-id fetcher.
...
src/
  babel/
    testIdPlugin.ts         — Adds data-mcp-id="Component:file:line" to capitalized JSX + emits __mcp_hooks metadata on component/custom-hook functions. Each hook entry carries the same shape mcpId="<name>:<shortFile>:<line>" so an agent can jump to the call from a fiber_tree query result. Maps all 18 React 16/18/19 stable hooks to dedicated kinds (Optimistic/ActionState/Use…); use(promise|context) is detected via a widened `use` exact + /^use[A-Z]/ regex with a React-namespace guard against `<obj>.use(...)` false positives. Runs on node_modules too.
  ...
  server/
    ...
    bridge.ts               — WS server, request/response dispatch, client identity. Sticky clientId: on close the entry moves to a 10-minute ghost map keyed by `(platform, deviceId, bundleId)`; a re-registering client matching all three reuses the same id (`ios-1` survives Fast Refresh / app-backgrounded / Xcode rebuild). Ghosts invisible to listClients while alive.
  ...
  modules/
    ...
    logBox/                 — LogBox overlay control (get / dismiss / clear by level / ignore / set_installed); dev-only surface
    navigation/             — React Navigation control + 100-entry history + screen component enrichment (navigate({mode}) / pop({to}) collapse the verb set)
    network/                — fetch + XMLHttpRequest interception with body cap + header/body redaction
    reactQuery/             — React Query cache inspection + mutate({ action: 'invalidate'|'refetch'|'remove'|'reset' })
  ...
  shared/
    ...
    projection/             — `projectValue` (path-based drill + ${kind} markers), `resolvePath`, `redact`
    rn/                     — RN interop: `core.ts` (getRN / loadRN / loadRNInternal), `deviceInfo.ts` (optional react-native-device-info loader)
  scripts/
...
- **Modules**: each module gets its own folder (e.g. `modules/navigation/`) with `<name>.ts`, `types.ts`, `index.ts`. Complex modules split utilities into sibling files — see `modules/fiberTree/` (13 files: `hooks.ts`, `query.ts`, `children.ts`, `projection.ts`, `finder.ts`, `waitFor.ts`, `viewport.ts`, `constants.ts`, `redact.ts`, `utils.ts`, `types.ts`, `index.ts`, `fiberTree.ts`).
...
- **`testIdPlugin`** (dev only, `react-native-mcp-kit/babel/test-id-plugin`): two transforms in one pass.
  ...
  2. Emits `Component.__mcp_hooks = [{ name, kind, hook, mcpId?, fn? }, ...]` after every function/arrow declaration that (a) has a capitalized name + JSX body OR a body containing hook calls (covers portal-like components that `return null`), or (b) has a `/^use([A-Z]|$)/` name + hook calls in body (custom hook or bare `use`). Each entry's `mcpId` carries `<varName>:<shortFile>:<line>` in the same shape as JSX `data-mcp-id` so an agent can jump from a fiber_tree hook result straight to the call site with `Read(file, line)`. HOC-wrapped components are unwrapped name-agnostically: any callee that's an `Identifier` (`memo`, `forwardRef`, `observer`, `withAuth`, …) or import-namespaced `MemberExpression` (`React.memo`) is treated as a wrapper, recursing into its first argument. The unwrap also walks transparently through `_c = arrow` / sequence-expression wraps that react-refresh injects, and through TS casts (`memo(...) as Type`, `<Foo>...`, `satisfies`, `!`). For `const Foo = anyHoc(InnerFn)` the plugin emits `Object.defineProperty(Foo, '__mcp_hooks', { get: () => InnerFn.__mcp_hooks })` (wrapped in try/catch for frozen / primitive returns) so the lookup is order/scope/wrapper-shape agnostic. Hook calls are matched as direct identifiers (`useState(...)`, bare `use(...)`) and MemberExpressions (`React.useState(...)`, `React.use(...)`) so bundle-compiled library hooks are covered. For the bare `<obj>.use(...)` form, only React-namespace objects (`React` / `_react` / `_React2` / …) pass; ambiguous `database.use(middleware)` / `app.use(router)` calls are filtered out so they don't show as false-positive hooks. `hook` records the source-level hook function name (`useState`, `useAnimatedStyle`, `use`); `fn` references the hook function when the callee resolves to a module-scoped binding — runtime reads `fn.__mcp_hooks` to recursively expand custom-hook sub-metadata. HOOK_KIND covers all 18 React 16/18/19 stable hooks (State, Effect, Memo, Callback, Ref, Context, Reducer, ImperativeHandle, LayoutEffect, InsertionEffect, DebugValue, Transition, DeferredValue, Id, SyncExternalStore, Optimistic, ActionState, Use); react-dom-only hooks (`useFormStatus`, `useFormState`) are intentionally omitted. All metadata writes are deferred to `Program:exit` via a per-file queue so they survive `replaceWith` calls from upstream plugins (notably react-refresh re-traversing rebuilt subtrees). Metro runs the plugin on node_modules by default, so `react-redux`, `@tanstack/react-query`, etc. get annotated automatically (first build pays a few-second cost; cached after).
