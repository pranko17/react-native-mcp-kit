...
Input injection (tap / swipe / type) on real iOS devices is not implemented — it needs a different DTX service or WebDriverAgent. On real iOS today: `host__screenshot` works (CoreDevice tunnel), `host__launch_app` / `host__restart_app` go through `xcrun devicectl device process launch` (restart folds into one call via `--terminate-existing`), and `host__terminate_app` returns an explicit error — devicectl can only terminate by PID, not bundle id. `host__list_devices` currently enumerates simulators + adb devices only (devicectl listing is used by the resolver, not the listing tool).
