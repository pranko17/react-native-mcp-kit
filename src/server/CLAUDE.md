...
- **Daemon death is survivable**: each proxy's backend shim keeps the MCP session up, respawns the daemon, reconnects, then re-broadcasts `list_changed`. Apps re-register on reconnect, so the catalog rebuilds itself — daemon state (sticky ghosts) is the only loss.
