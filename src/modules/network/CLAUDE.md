...
- Matching dimensions: url (substring / `/regex/`), `method`, `times`, and request-body constraints — `bodyContains` (substring / `/regex/` over the raw serialized body) and `bodyMatch` (dot-paths with `[0]` / `[-1]` indices into the JSON-parsed body; primitive = strict equality, `{ contains }` / `{ regex }` for strings, objects/arrays deep-equal, entries AND-ed). Body constraints separate requests that share a URL and differ only in the payload; they never match bodyless or non-string-body requests.
